...
const EN: Labels = {
  brand: "LENSAHAM MARKET INTELLIGENCE",
...
const ID: Labels = {
  brand: "LENSAHAM INTELIJEN PASAR",
  ...
  return (
    <Page size="A4" style={s.coverPage}>
      <View style={s.coverInner}>
        ...
        <View style={s.coverMetricsRow}>
          <View style={s.coverMetric}>
            ...
            <Text style={[s.coverMetricValue, { color: GREEN }]}>{ov.advancingCount}</Text>
          </View>
          <View style={s.coverMetric}>
            <Text style={s.coverMetricLabel}>{l.declining}</Text>
            <Text style={[s.coverMetricValue, { color: RED }]}>{ov.decliningCount}</Text>
          </View>
          <View style={s.coverMetric}>
            <Text style={s.coverMetricLabel}>{l.unchanged}</Text>
            <Text style={[s.coverMetricValue, { color: GRAY }]}>{ov.unchangedCount}</Text>
          </View>
          <View style={s.coverMetric}>
            <Text style={s.coverMetricLabel}>{l.totalValue}</Text>
      ...
      <View style={s.coverFooter}>
        ...
        <Text style={[s.coverDisclaimer, { textAlign: "right" }]}>lensaham.com</Text>
      </View>
    </Page>
  );
}
    ...
    >
      ...
      <Page size="A4" style={s.page} wrap>
        ...
        <View style={s.sectionWrap}>
          ...
          <View style={s.tableHeader}>
            <Text style={[s.tableHeaderCell, { width: "30%" }]}>{l.sector}</Text>
            <Text style={[s.tableHeaderCell, { width: "14%", textAlign: "right" }]}>{l.change}</Text>
            <Text style={[s.tableHeaderCell, { width: "14%" }]}>{l.topStock}</Text>
            <Text style={[s.tableHeaderCell, { width: "14%", textAlign: "right" }]}>%</Text>
            <Text style={[s.tableHeaderCell, { width: "14%" }]}>{l.sentiment}</Text>
          </View>
          {report.sectorPerformance?.map((sp: SectorPerformance, i: number) => (
            <View key={i} style={[s.tableRow, i % 2 === 1 ? s.tableRowAlt : {}]} wrap={false}>
              <Text style={[s.tableCellBold, { width: "30%" }]}>{sp.sector}</Text>
              <Text style={[s.tableCell, { width: "14%", textAlign: "right" }, sp.change >= 0 ? s.positive : s.negative]}>
                {fmtPct(sp.change)}
              </Text>
              <Text style={[s.tableCellBold, { width: "14%" }]}>{sp.topStock}</Text>
              <Text style={[s.tableCell, { width: "14%", textAlign: "right" }, sp.topStockChange >= 0 ? s.positive : s.negative]}>
            ...
            <View style={s.col}>
              ...
              {report.foreignFlow?.topBought?.map((f, i) => (
                <View key={i} style={[s.tableRow, i % 2 === 1 ? s.tableRowAlt : {}]} wrap={false}>
                  ...
                  <Text style={[s.tableCell, s.positive, { width: "35%", textAlign: "right" }]}>{fmtVal(f.netBuy)}</Text>
                </View>
              ))}
            </View>
            <View style={s.col}>
              ...
              {report.foreignFlow?.topSold?.map((f, i) => (
                <View key={i} style={[s.tableRow, i % 2 === 1 ? s.tableRowAlt : {}]} wrap={false}>
                  <Text style={[s.tableCellBold, { width: "25%" }]}>{f.code}</Text>
                  <Text style={[s.tableCell, { width: "40%" }]}>{f.name}</Text>
                  <Text style={[s.tableCell, s.negative, { width: "35%", textAlign: "right" }]}>{fmtVal(f.netSell)}</Text>
                </View>
              ))}
            </View>
          </View>
